...
class ImageService:
    def __init__(self, provider, api_key=None, proxy=None, hf_key=None):
        ...
        logger.info(f'🎨 ImageService: {self.provider.upper()} (Multi-Style v9.1)')
    ...
    async def _generate_hf(self, query, style):
        ...
        # Комбинируем запрос + стиль
        enhanced_prompt = f"{query}, {style}, crypto currency theme, no text, highly detailed"
